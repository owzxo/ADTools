...
#include "Factories/FbxImportUI.h" // FBX loader parameters
#include "Factories/FbxStaticMeshImportData.h" // FBX loader parameters
...
TArray<TSharedPtr<FJsonValue>> GetVectorJsonData(FVector UEVector)
{
	...
	UEVectorArray.Add(MakeShareable(new FJsonValueNumber(UEVector.X)));
	UEVectorArray.Add(MakeShareable(new FJsonValueNumber(UEVector.Y)));
	UEVectorArray.Add(MakeShareable(new FJsonValueNumber(UEVector.Z)));
...
TArray<TSharedPtr<FJsonValue>> GetRotationJsonData(FRotator ActorRotation)
{
	...
	RototationArray.Add(MakeShareable(new FJsonValueNumber(ActorRotation.Roll)));
	RototationArray.Add(MakeShareable(new FJsonValueNumber(ActorRotation.Pitch)));
	RototationArray.Add(MakeShareable(new FJsonValueNumber(ActorRotation.Yaw)));
...
{
	const auto StaticMeshActor = GetActorByName<AActor>(ObjectName.TrimStartAndEnd(),World);
	if(StaticMeshActor)
	{
...
void TranslateActorByName(FString ObjectName, FVector NewLocation, UWorld* World)
{
	...
	{
		StaticMeshActor->Modify();//reg9ster undo
...
void TranslateAndRaytraceActorByName(FString ObjectName, FVector NewLocation, float RaytraceDistance, float MaxNormalDeviation, TArray<AActor*> IgnoreActors, UWorld* World)
{
	...
	if(Actor!=nullptr)
	{
		...
		if(RaytraceDistance)
		{
			FVector RayDirection = FVector(0,0,RaytraceDistance);
			FVector StartPoint = NewLocation;
			FVector EndPoint = NewLocation - RayDirection;
			FHitResult TraceResult = RayTrace(StartPoint,EndPoint,World,IgnoreActors);
			//设置位置.
			FVector HitLocation = TraceResult.Location; //如果没有找到，将返回全部零
			if(!HitLocation.IsZero())
			{
				NewLocation = HitLocation;
				NewLocation.Z +=FMath::FRandRange(0,0.25f);
			}

			float UpDotProduct = FVector::DotProduct(FVector(0,0,1),TraceResult.Normal);
			UE_LOG(LogADTools,Display,TEXT("DOT PRODUCT:%f, %f"),UpDotProduct,MaxNormalDeviation);
...
FQuat RotationFromNormal(FVector NormalVector)
{
	...
	const float DotProduct = FVector::DotProduct(UpVector,NormalVector);
...
void RelativeTranslateActorByName(FString ObjectName, FVector LocationDelta, UWorld* World)
{
	...
	{
		StaticMeshActor->Modify(); //register undo
		StaticMeshActor->AddActorWorldOffset(LocationDelta,false);
	}
}
...
void RotateActorByName(FString ObjectName, FVector NewRotationVec, UWorld* World)
{
	...
	if(StaticMeshActor !=nullptr)
	{
		const FRotator NewRotation = FRotatorFromXYZVec(NewRotationVec);
		StaticMeshActor->Modify();
		StaticMeshActor->SetActorRotation(NewRotation);
	}
}
...
void RelativeRotateActorsByName(const TArray<FString>& ObjectNames, FVector NewRotationVec, UWorld* World)
{
	for(auto& ObjectName:ObjectNames)
	{
		RelationRotateActorByName(ObjectName,NewRotationVec,World);
	}
}
...
void RelativeScaleActorByName(FString ObjectName, FVector NewScale, UWorld* World)
{
	...
	if (StaticMeshActor != nullptr)
	{
		const FVector OldScale = StaticMeshActor->GetActorScale3D();
		StaticMeshActor->Modify(); // register undo
...
{
	const auto RemoveActor = GetActorByName<AActor>(ObjectName.TrimStartAndEnd(), World);
...
void RemoveChildrenRecursive(AActor* Actor)
{
	...
	for (const auto & RemoveActor : DescendentPointers)
	{
...
{
	const auto TopActor = GetActorByName<AActor>(ObjectName.TrimStartAndEnd(), World);
	if (TopActor != nullptr)
	{	
		TArray<AActor*> DescendentPointers;
...
FString GetValidActorsExpandedTransformJsonDict(const TArray<AActor*>& ValidActors, UWorld* World)
{
	...
	for (auto& Valid : ValidActors)
	{
		...
		TArray<TSharedPtr<FJsonValue>> MeshTransformArray = GetVectorJsonData(Valid->GetActorLocation());
		// rotation
		const FRotator ActorRotation = Valid->GetActorRotation();  // reused later to reset rotation back after getting local space bounds
		TArray<TSharedPtr<FJsonValue>> MeshRotationArray = GetRotationJsonData(Valid->GetActorRotation());		
		MeshTransformArray.Append(MeshRotationArray);		
		// scale
		TArray<TSharedPtr<FJsonValue>> MeshScaleArray = GetVectorJsonData(Valid->GetActorScale3D());
		MeshTransformArray.Append(MeshScaleArray);
		// size
		const USceneComponent* SceneComponent = GetValidSceneComponent(Valid);
		if (SceneComponent != nullptr)
		{
			Valid->SetActorRotation(FRotator(0.0, 0.0, 0.0));  // reset rotation to get accurate LS size of arbitrary actor
			const FVector MeshSize = SceneComponent->Bounds.GetBox().GetSize();  // - this would be world space, if wasn't for rotation
...
FString GetCameraInfoJsonDict(const bool ObjectsOnScreen)
{
	const TSharedPtr<FJsonObject> RootObject = MakeShared<FJsonObject>();
	// get data
	...
	const FVector EditorCameraDirection = Client->GetViewRotation().Vector();	
	const float FOV = Client->ViewFOV;
	// do the json struct hubris
	// location
	const TArray<TSharedPtr<FJsonValue>> CameraLocationArray = GetVectorJsonData(EditorCameraPosition);	
	RootObject->SetArrayField("camera_location", CameraLocationArray);
	// direction
	const TArray<TSharedPtr<FJsonValue>> CameraDirectionArray = GetVectorJsonData(EditorCameraDirection);
	...
	if (ObjectsOnScreen)
	{
		const TArray<AActor*> RenderedActors = GetRenderedValidActors();
...
{
	const auto FoundActor = GetActorByName<AActor>(ActorName, World);
...
TArray<AActor*> GetWorldValidActors(const UWorld* World)
{
	...
	{
		const auto Validated = ValidateActor(*ActorItr);
...
{
	TArray<AActor*> ValidActors;
	for (const auto& RenderedActor : GetRenderedActors())
	{
...
TArray<AActor*> GetValidActorsByMeshAssetPath(const FString& MeshAssetPath, const UWorld* World)
{
	...
	for (TActorIterator<AActor> ActorItr(World); ActorItr; ++ActorItr)
	{
		...
		const UStaticMeshComponent* StaticMeshComponent = Cast<UStaticMeshComponent>(GetValidMeshComponent(*ActorItr));
...
void SetVecMatAttrsForValidActor(AActor* Actor, const FString AttributeName, const FLinearColor Value, const int Index = -1)
{
	TArray<UMaterialInterface*> Materials = GetAllMaterialInterfacesFromValidActor(Actor);
	UE_LOG(LogADTools, Display, TEXT("Index %i"), Index);
	if (Index >= 0)  // if no index specified carry on as usual
	{
		UE_LOG(LogADTools, Display, TEXT("Material Num %i"), Materials.Num());
		if (Materials.Num() <= Index)
		{
			UE_LOG(LogADTools, Display, TEXT("Material index %i out of range in %s"), Index, *Actor->GetPathName());
			return;
		}			
	...
	for (UMaterialInterface* Material : Materials)
	{
		if (UMaterialInstanceConstant* MaterialInstance = Cast<UMaterialInstanceConstant>(Material))  // input is material interface
		{
			...
			if (!AttrFound)
				UE_LOG(LogADTools, Display, TEXT("Attribute %s not found in %s"), *AttributeName, *MaterialInstance->GetPathName());			
		}
		else		
			UE_LOG(LogADTools, Display, TEXT("Material %s not an instance"), *Material->GetPathName());
	}
}
...
{	
	Actor->Modify();
	const auto MeshComponent = GetValidMeshComponent(Actor);
...
TArray<FLinearColor> GetVecMatAttrsForValidActor(AActor* ValidActor, const FString& AttributeName, int Index = -1)
{
	...
	if (Index >= 0)  // if no index specified carry on as usual
	{
		...
		if (Materials.Num() <= Index)
		{
			UE_LOG(LogADTools, Display, TEXT("Material index %i out of range in %s"), Index, *ValidActor->GetPathName());
			return OutVectors;
		}
		else  // if index is specified check if there is enough indexes and then try to set. !Warning assuming ParameterInfo matches visible order in editor
		{
	...
	for (UMaterialInterface* Material : Materials)
	{
		if (const UMaterialInstanceConstant* MaterialInstance = Cast<UMaterialInstanceConstant>(Material))  // input is material interface
		{
			TArray<FMaterialParameterInfo> OutParameterInfo;
			TArray<FGuid> OutParameterIds;
			Material->GetAllVectorParameterInfo(OutParameterInfo, OutParameterIds);
...
TArray<FString> GetMaterialAttributeNamesFromValidActors(const TArray<FString>& ActorNames, FString AttributeType, UWorld* World)
{
	...
	{
		const auto ValidActor = GetValidActorByName(ActorName, World);
		if (ValidActor)
		{
...
TArray<FString> GetMaterialAttributeNamesFromValidActor(AActor* Actor, FString AttributeType)
{
	...
	for (const UMaterialInterface* Material : Materials)
	{
		...
		{
			UE_LOG(LogADTools, Display, TEXT("Request for an unknown material attribute type: %s"), *AttributeType);
		}
...
{
	const TSharedPtr<FJsonObject> JsonDict = MakeShared<FJsonObject>();
	for (FString ActorName : ActorNames)
	{
		...
		if (FoundActor != nullptr)
		{
			...
			UE_LOG(LogADTools, Display, TEXT("Materials used by actor '%s':"), *ActorName);
			for (auto& MaterialName : MaterialNames)
			{
				JsonArray.Add(MakeShareable(new FJsonValueString(MaterialName)));
				UE_LOG(LogADTools, Display, TEXT("%s"), *MaterialName);
			}
...
void SetMeshAssetMaterialFromJsonString(const FString& JsonString)
{
	...
	for (const auto& JsonMeshObject : JsonObject->Values) {
		auto JsonMaterialObjects = JsonMeshObject.Value->AsArray();
		for (const auto& JsonMaterialObject : JsonMaterialObjects)
		{
			...
			if (Parameters.Num() >= 2)
			{
				...
				const auto MeshAsset = LoadObject<UStaticMesh>(nullptr, *MeshAssetPath);
				if (MeshAsset != nullptr)
				{

					if (MaterialIndex >= MeshAsset->GetStaticMaterials().Num())
					{
						UE_LOG(LogADTools, Warning, TEXT("set_mesh_asset_material: provided material index (%d) is out of range for asset: %s"), MaterialIndex, *MeshAssetPath);
					}
					const auto MaterialAsset = LoadObject<UMaterialInterface>(nullptr, *TargetMaterialPath);
...
{
	const auto Actor = GetActorByName<AActor>(ActorName, World);
	const auto LearningSceneComponent = GetValidSceneComponent(Actor); // this function returns child static mesh components which are managed by standalone.
	const auto PrimitiveMeshComponent = Cast<UPrimitiveComponent>(LearningSceneComponent);
	if (PrimitiveMeshComponent != nullptr)
	{
		const auto DefaultObject = PrimitiveMeshComponent->GetClass()->GetDefaultObject<UPrimitiveComponent>();
...
FString GetActorsPhysicsSimulationStateByName(const TArray<FString>& ActorNames)
{
	...
	for (auto ActorName : ActorNames)
	{
		const auto Actor = GetActorByName<AActor>(ActorName, GetEditorWorld());		
		if (Actor)
		{
...
{
	const TArray<AActor*> SimulatingActors = GetSimulatingActorsByName(ActorNames);
...
void DisablePlayInEditor()
{
	if (GEditor->PlayWorld != NULL)
	{
		try
		{
			...
			UE_LOG(LogADTools, Display, TEXT("Succesful Request End Play Map"));
		}
		catch (const std::exception& e)
		{
			UE_LOG(LogADTools, Display, TEXT("Error Requesting End Play Map: "), *e.what());
		}
	}
}
...
TArray<AStaticMeshActor*> AddStaticMeshActorOnSelection(FString MeshPath, UWorld* World)
{
	...
	{
		AStaticMeshActor* NewActor = AddStaticMeshActor(World, MeshPath, Name, FVector(0, 0, 0), Rotation, Scale);		
		if (NewActor != nullptr)
		{
			NewActors.Add(NewActor);
		}
...
{	
	...
	* */	
	...
	UE_LOG(LogADTools, Display, TEXT("Adding blueprint: %s"), *MeshPath);
	if (BPSubClass == nullptr)
	{
		UE_LOG(LogADTools, Display, TEXT("Failed to load blueprint: %s"), *MeshPath);
		return nullptr;
	}
	UE_LOG(LogADTools, Display, TEXT("1"));
	const FRotator Rotation = FRotatorFromXYZVec(RotationVec);
	FActorSpawnParameters SpawnInfo;  //	SpawnInfo.Name = FName(*Name);  // didn't seem to work well. deletes the object with original name from scene but doesn't assign the name %/ 		
	SpawnInfo.SpawnCollisionHandlingOverride = ESpawnActorCollisionHandlingMethod::AlwaysSpawn;
	// const FVector ConstLocation = Location;
	UE_LOG(LogADTools, Display, TEXT("2"));
	...
	UE_LOG(LogADTools, Display, TEXT("3"));
	if (obj != nullptr)
	{
		obj->SetActorScale3D(Scale);
		if (Name != "")
		{
			UE_LOG(LogADTools, Display, TEXT("4"));
		...
		GEditor->SelectActor(obj, true, false);  // add to selection, first true is important
		UE_LOG(LogADTools, Display, TEXT("5"));
	}
	return obj;
}
...
{
	const FString FullMeshReference = "StaticMesh'" + MeshPath + "'";  // StaticMesh'/Game/StarterContent/Props/SM_Chair.SM_Chair'
	...
	{
		UE_LOG(LogADTools, Display, TEXT("Warning! Static Mesh Not Found: %s"), *FullMeshReference);
		return nullptr;
	}
}
...
void SetStaticMeshAssetForValidActor(AActor* ValidActor, FString MeshPath)
{
	...
	if (MeshAsset)
	{
		const auto StaticMesh = GetValidStaticMeshComponent(ValidActor);
		StaticMesh->Modify();
...
ActorType* GetActorByName(const FString& ActorName, UWorld* World)
{
	...
	UE_LOG(LogADTools, Display, TEXT("%s: '%s' not found in the scene"), *(ActorType::StaticClass()->GetFName().ToString()), *ActorName);
	return nullptr;
}
...
void SelectSceneActorsByName(const TArray<FString>& ObjectNames)
{
	...
	for (auto& ObjectName : ObjectNames)
	{
		UE_LOG(LogADTools, Display, TEXT("Select: %s"), *ObjectName);
...
{
	const auto Object = GetActorByName<AActor>(ObjectName.TrimStartAndEnd(), World);	
	FString NoParentName = TEXT("No_Parent");  // Warning! Has to be consistent with other DCCs
	if (Object)
	{
		//auto Parent = Object->GetParentActor();
		const auto Parent = Object->GetAttachParentActor();
		if (Parent)
		{
...
{
	const auto Object = GetActorByName<AActor>(Name.TrimStartAndEnd(), World);
	if (Object)
...
void FixObjectNames(const UWorld* World)
{
	...
	for (const auto& ValidActor : AllValidActors)
	{		
...
void RenameActor(FString SourceName, FString TargetName, UWorld* World)
{
	...
	const auto ActorToRename = GetActorByName<AActor>(SourceName.TrimStartAndEnd(), World);	
	if (ActorToRename != nullptr)
	{
		...
		// Rename
		const FString CurrentName = ActorToRename->GetName();
...
void UnloadLevel(FString LevelPath, const UWorld* World)
{
	...
	const int DotIndex = LevelPath.Find(".");
	if (DotIndex > 0) 
	{
	...
	for (const auto& CurrentLevel : CurrentLevels) 
	{		
	...
	UE_LOG(LogADTools, Display, TEXT("Couldn't unload %s"), *LevelPath);
}
...
void SetLevelVisibility(FString LevelPath, bool IsVisible, const UWorld* World)
{
	...
	UE_LOG(LogADTools, Display, TEXT("Couldn't set level visibility: %s"), *LevelPath);
}
...
void SetLevelCurrent(FString LevelPath, UWorld* World)
{
	...
	UE_LOG(LogADTools, Display, TEXT("Couldn't set level current: %s"), *LevelPath);
	return;
}
...
FString GetJsonAssetDataFromAssetPaths(TArray<FString> AssetReferencePaths)
{
	...
	for (const FString& AssetReferencePath : AssetReferencePaths)
	{
		...
		if (!AssetData.IsValid())
		{
			...
			{
				UE_LOG(LogADTools, Display, TEXT("No asset found for %s"), *AssetReferencePath);
	...
	for (const FAssetData& SelectionAssetData : Assets)
	{
		...
		{
			UE_LOG(LogADTools, Display, TEXT("Unsupported data type"));
...
void EditAsset(FString AssetReferencePath)
{
	...
	UObject* Asset = StaticLoadObject(Class, nullptr, *AssetReferencePath);
	if (Asset)
	{
...
{
	TArray <UObject*> Assets;
	for (const auto & AssetReferencePath : AssetReferencePaths)
	{
		UObject* Asset = GetAssetUObject(AssetReferencePath);
		if (Asset)
		{
...
void ImportAssetToCurrentPath()
{
	...
	if (Selections.Num() > 0)
	{
		const FName CurrentPath = Selections[0].PackagePath;  // import at first selected object
...
static UObject* ImportObjectUsingFactory(UFactory& Factory, const FString& AbsoluteSourcePath, const FString& AssetTarget)
{
	...
	const FString AssetName = FPaths::GetBaseFilename(AssetTarget);
	...
	const auto AssetObject = Factory.ImportObject(Factory.ResolveSupportedClass(), Package, FName(*AssetName), RF_Public | RF_Standalone, AbsoluteSourcePath, nullptr, Canceled);
	if (Canceled == true)
	{
		UE_LOG(LogADTools, Display, TEXT("Import canceled."));
		return nullptr;
	}
...
TArray<UObject*> ImportFBXAssetsFromJsonString(const FString& JsonString)
{
	...
	TSharedPtr<FJsonObject> JsonObject;
	if (ParseJsonString(JsonString, JsonObject)) {
		for (const auto& JsonItem : JsonObject->Values) {
...
UObject* LoadFBXFile(const FString& AbsoluteSourcePath, const FString& AssetTarget) {
	UE_LOG(LogADTools, Display, TEXT("Promethean Plugin is loading FBX file: %s to %s"), *AbsoluteSourcePath, *AssetTarget);
...
{
	UE_LOG(LogADTools, Display, TEXT("Promethean Plugin is loading Texture file: %s to %s"), *AbsoluteSourcePath, *AssetTarget);
	// Create Texture factory, which handles all types of files.
	UTextureFactory* Factory = NewObject<UTextureFactory>(UTextureFactory::StaticClass(), FName("Factory"), RF_NoFlags);
...
FString RayTraceFromActorsByNameJson(const TArray<FString>& ActorNames, FVector RayDirection, UWorld* World)
{
	...
	for (auto& ActorName : ActorNames)
	{
		const FVector Vector = RayTraceFromActorByName(ActorName, RayDirection, World);
		TArray<TSharedPtr<FJsonValue>> JsonArray;		
...
FHitResult RayTrace(FVector StartPoint, FVector EndPoint, UWorld* World, TArray<AActor*> IgnoreActors)
{
	...
	bHit = World->LineTraceSingleByObjectType(HitResult, StartPoint, EndPoint, CollisionParams, TraceParams);
...
FString RayTraceMultiFromActorsByNameJson(const TArray<FString>& ActorNames, FVector RayDirection, UWorld* World)
{
	...
	{
		const FVector Vector = RayTraceMultiFromActor(Actor, RayDirection, World, Actors);
		TArray<TSharedPtr<FJsonValue>> JsonArray;
		TSharedRef<FJsonValueNumber> X = MakeShareable(new FJsonValueNumber(Vector.X));
		TSharedRef<FJsonValueNumber> Y = MakeShareable(new FJsonValueNumber(Vector.Y));
		TSharedRef<FJsonValueNumber> Z = MakeShareable(new FJsonValueNumber(Vector.Z));
...
FVector RayTraceMultiFromActor(AActor* Actor, FVector RayDirection, UWorld* World, TArray<AActor*> IgnoreActors)
{
	...
	const FVector HalfDirectionRay = RayDirection * 0.5;
	...
	const FVector StartPoint = ActorPivot - HalfDirectionRay;  // multi ray is by default ray direction based (no backfaces), so instead of shooting 2 rays one up and one down...
	const FVector EndPoint = ActorPivot + HalfDirectionRay;  //... we shoot one from higher up and find closest to original 
	...
	if (TraceResults.Num() > 0)
	{
		...
		{
			const float CurrentDistance = FVector::Distance(TraceResult.Location, ActorPivot);
...
TArray<FVector> GetMeshTrianglePositionsFromActor(AActor* MeshActor)
{
	...
	const auto ValidMeshComponent = GetValidStaticMeshComponent(MeshActor);
...
{	
	...
	for (auto& SectionIt : LODModel.Sections)
	{
		...
		for (uint32 TriIndex = 0; TriIndex < Section.NumTriangles; TriIndex++) // if (InTriangleIndex >= SectionFirstTriIndex && InTriangleIndex < NextSectionTriIndex)		
		{
			...
			const int32 IndexBufferIdx = TriIndex * 3 + Section.FirstIndex;
...
TArray<TSharedPtr<FJsonValue>> FVectorArrayToJsonArray(const TArray<FVector>& VectorArray)
{
	...
	const TSharedRef<FJsonValueObject> JsonDictValue = MakeShared<FJsonValueObject>(JsonDict);
	int32 i = 0;
	for (const FVector Vector : VectorArray)
	{
		i += 1;
		TArray<TSharedPtr<FJsonValue>> MeshScaleArray = FVectorToJsonArray(Vector);
...
{
	const TArray<TSharedPtr<FJsonValue>> JsonDictArray = FVectorArrayToJsonArray(VectorArray);  // for output		
...
{
	UE_LOG(LogADTools, Display, TEXT("Capture screenshot at %s"), *OutputPath);
...
{	
	...
	const FString OutputFolder = OutputPath.Left(Index) + "/";  // need to check if the folder exists

	UE_LOG(LogADTools, Display, TEXT("Saving file: %s"), *OutputFolder);
	constexpr bool AllowOverwriting = true;
	...
	if (PlatformFile.CreateDirectoryTree(*OutputFolder))
	{
		...
		if constexpr (AllowOverwriting || !FPaths::FileExists(*OutputPath))
		{
			FFileHelper::SaveStringToFile(TextToSave, *OutputPath);
			UE_LOG(LogADTools, Display, TEXT("File saved to: %s"), *OutputPath);
		}
	}
}
...
FTransform StringArrayToTransform(const TArray<FString>& StringArray)
{
	...
	{
		const FVector Translation = FVector(FCString::Atof(*StringArray[0]), FCString::Atof(*StringArray[1]), FCString::Atof(*StringArray[2]));
		const FRotator Rotation = FRotatorFromXYZVec(FVector(FCString::Atof(*StringArray[3]), FCString::Atof(*StringArray[4]), FCString::Atof(*StringArray[5])));
		const FVector Scale = FVector(FCString::Atof(*StringArray[6]), FCString::Atof(*StringArray[7]), FCString::Atof(*StringArray[8]));
...
TArray<FString> ActorArrayToAssetPathsArray(const TArray<AActor*>& Actors)
{
	...
	for (auto& Actor : Actors)
	{
		const UStaticMeshComponent* StaticMeshComponent = Cast<UStaticMeshComponent>(GetValidSceneComponent(Actor));
		if (StaticMeshComponent != nullptr)
		{
			const UStaticMesh* StaticMesh = StaticMeshComponent->GetStaticMesh();
			PathArray.Add(StaticMesh->GetPathName());  // .ToLower()  - don't lower_case because that can affect hash consistency for DB lookup
		}
		else  // WARNING! NEED TO MAINTIAIN INDEX ORDER WITH ARRAY LIST IN CASE NAMES AND PATH WILL BE MATCH BY INDEX. So adding empty string for path here
		{
			PathArray.Add(TEXT("none"));  // json write crashes when trying to serialize empty string, so using 'none'
		}
	}
	return PathArray;
}
...
FString StaticMeshActorArrayToAssetPathsString(const TArray<AStaticMeshActor*>& Actors)
{
	...
	for (auto& Actor : Actors)
	{
		TArray<UStaticMeshComponent*> ActorStaticMeshComponents;
		Actor->GetComponents<UStaticMeshComponent>(ActorStaticMeshComponents);
		for (const UStaticMeshComponent* StaticMeshComponent : ActorStaticMeshComponents)
		{
			const UStaticMesh* StaticMesh = StaticMeshComponent->GetStaticMesh();
...
bool ParseJsonString(const FString& String, TSharedPtr<FJsonObject>& Result)
{
	...
	if (!FJsonSerializer::Deserialize(static_cast<TSharedRef<TJsonReader<TCHAR>>>(JsonReader), Result))
	{
		UE_LOG(LogADTools, Display, TEXT("Promethean TCP command json parsing error: %s"), *JsonReader->GetErrorMessage());
		return false;
	}
	return true;
}
